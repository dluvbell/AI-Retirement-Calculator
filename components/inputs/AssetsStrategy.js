...
const handleSimpleModeClick = () => {
    ...
    // ★★★ [수정] LIRA/LIF도 override를 false로 설정 ★★★
    onUpdate('advancedSettings', { ...scenario.settings.advancedSettings, 
        tfsa: { ...scenario.settings.advancedSettings.tfsa, override: false }, 
        rrsp: { ...scenario.settings.advancedSettings.rrsp, override: false }, 
        nonReg: { ...scenario.settings.advancedSettings.nonReg, override: false },
        lira: { ...scenario.settings.advancedSettings.lira, override: false }, 
        lif: { ...scenario.settings.advancedSettings.lif, override: false } 
    });
};
...
const handleAdvancedModeClick = () => {
    ...
    // ★★★ [수정] LIRA/LIF도 override를 true로 설정 ★★★
    onUpdate('advancedSettings', { ...scenario.settings.advancedSettings, 
        tfsa: { ...scenario.settings.advancedSettings.tfsa, override: true }, 
        rrsp: { ...scenario.settings.advancedSettings.rrsp, override: true }, 
        nonReg: { ...scenario.settings.advancedSettings.nonReg, override: true },
        lira: { ...scenario.settings.advancedSettings.lira, override: true }, 
        lif: { ...scenario.settings.advancedSettings.lif, override: true } 
    });
};
...
const assetKeys = ['growth', 'balanced', 'dividend_can', 'dividend_us', 'bond', 'gic'];
// ★★★ [수정] LIRA, LIF 계좌 키 추가 ★★★
const accountKeys = ['rrsp', 'tfsa', 'nonReg', 'lira', 'lif']; 
            ...
            <div>
                <h4 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '12px', borderBottom: '1px solid #374151', paddingBottom: '8px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px', alignItems: 'end' }}>
                        ...
                        {/* ★★★ [신설] LIRA Total 입력 필드 ★★★ */}
                        <div>
                            <label style={labelStyle} htmlFor="liraTotal">LIRA Total</label>
                            <input
                                type="number"
                                id="liraTotal"
                                style={inputStyle}
                                value={scenario.settings.initialBalances.lira}
                                onChange={(e) => onUpdate('initialBalances', { ...scenario.settings.initialBalances, lira: parseFloat(e.target.value) || 0 })}
                            />
                        </div>
                        {/* ★★★ [신설] LIF Total 입력 필드 ★★★ */}
                        <div>
                            <label style={labelStyle} htmlFor="lifTotal">LIF Total</label>
                            <input
                                type="number"
                                id="lifTotal"
                                style={inputStyle}
                                value={scenario.settings.initialBalances.lif}
                                onChange={(e) => onUpdate('initialBalances', { ...scenario.settings.initialBalances, lif: parseFloat(e.target.value) || 0 })}
                            />
                        </div>
                        {/* --- [추가] ACB 비율 입력 필드 --- */}
                        <div>
