...
const BasicSettings = ({ scenario, onUpdate }) => {
    ...
    const spouseSettings = scenario.settings.spouseSettings || { 
        enabled: false, 
        birthYear: birthYear, 
        cppAmount: 0, cppStartAge: 65,
        oasAmount: 0, oasStartAge: 65,
        baseIncome: 0, baseIncomeStartAge: 65, baseIncomeEndAge: 95
    };
    
    // ★★★ [신규] 배우자 Locked-in 설정 가져오기 ★★★
    ...
    // 배우자 Locked-in 설정 핸들러
    const handleSpouseLockedInChange = (field, value) => {
        const numericValue = parseFloat(value) || 0;
        onUpdate('spouseLockedIn', {
            ...spouseLockedIn,
            [field]: numericValue
        });
    };
        ...
        <div style={{ backgroundColor: '#1f2937', padding: '20px', borderRadius: '8px' }}>
            ...
            {spouseSettings.enabled && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px', marginBottom: '10px' }}>
                    ...
                    {/* ★★★ [수정] CPP Amount & Start Age ★★★ */}
                    <div>
                        <label style={labelStyle}>Spouse CPP (Amount / Start Age)</label>
                        <div style={{display: 'flex', gap: '8px'}}>
                            ...
                            <input
                                ...
                                value={spouseSettings.cppStartAge || 65}
                                onChange={(e) => handleSpouseChange('cppStartAge', e.target.value)}
                            />
                        </div>
                    </div>

                    {/* ★★★ [수정] OAS Amount & Start Age ★★★ */}
                    <div>
                        <label style={labelStyle}>Spouse OAS (Amount / Start Age)</label>
                        <div style={{display: 'flex', gap: '8px'}}>
                            <input
                                type="number"
                                placeholder="Amount ($)"
                            ...
                            <input
                                ...
                                placeholder="Age"
                                style={{...inputStyle, width: '80px'}}
                                value={spouseSettings.oasStartAge || 65}
                                onChange={(e) => handleSpouseChange('oasStartAge', e.target.value)}
                            />
                        </div>
                    </div>

                    {/* ★★★ [수정] Base Income Amount & Start/End Age ★★★ */}
                    <div style={{gridColumn: 'span 2'}}>
                        <label style={labelStyle}>Spouse Other Income (Amount / Start Age / End Age)</label>
                        <Tooltip text="Taxable pension or other income. Start/End age determines when this income is active.">
                            <span style={{marginLeft: '4px', fontSize: '12px', color: '#9ca3af', display:'inline-block', marginBottom:'4px'}}>(Details)</span>
                        </Tooltip>
                        <div style={{display: 'flex', gap: '8px'}}>
                            <input
                                type="number"
                            ...
                            <input
                                ...
                                placeholder="Start"
                                style={{...inputStyle, width: '80px'}}
                                value={spouseSettings.baseIncomeStartAge || 65}
                                onChange={(e) => handleSpouseChange('baseIncomeStartAge', e.target.value)}
                            />
                            <input
                                type="number"
                                placeholder="End"
                                style={{...inputStyle, width: '80px'}}
                                value={spouseSettings.baseIncomeEndAge || 95}
                                onChange={(e) => handleSpouseChange('baseIncomeEndAge', e.target.value)}
                            />
                        </div>
                    </div>
                </div>
            ...
            {/* --- Client Locked-in Settings --- */}
            <h3 style={sectionHeaderStyle}>
                Locked-in Account Settings (Client)
            </h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px' }}>
                <div>
            ...
            {/* ★★★ [신규] Spouse Locked-in Settings (활성화 시만 표시) ★★★ */}
            {spouseSettings.enabled && (
                <>
                    <h3 style={sectionHeaderStyle}>
                        Locked-in Account Settings (Spouse)
                    </h3>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px' }}>
                        <div>
                            <label style={labelStyle}>LIRA Conversion Age</label>
                            <input
                                type="number"
                                style={inputStyle}
                                value={spouseLockedIn.conversionAge}
                                onChange={(e) => handleSpouseLockedInChange('conversionAge', e.target.value)}
                            />
                        </div>
                        
                        <div>
                            <label style={labelStyle}>Unlocking % (to RRSP)</label>
                            <input
                                type="number"
                                style={inputStyle}
                                value={spouseLockedIn.unlockingPercent}
                                onChange={(e) => handleSpouseLockedInChange('unlockingPercent', e.target.value)}
                            />
                        </div>
                    </div>
                </>
